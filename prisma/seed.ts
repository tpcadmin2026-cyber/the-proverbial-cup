...
const DEFAULT_SETTINGS = [
  ...
  // Payments
  { key: 'payments.currency', group: 'payments', label: 'Currency', helpText: 'The currency used for all prices and Stripe checkout.', inputType: 'select', value: '"USD"', options: JSON.stringify([{ value: 'USD', label: 'USD — US Dollar ($)' }, { value: 'CAD', label: 'CAD — Canadian Dollar (CA$)' }, { value: 'GBP', label: 'GBP — British Pound (£)' }, { value: 'EUR', label: 'EUR — Euro (€)' }, { value: 'AUD', label: 'AUD — Australian Dollar (A$)' }, { value: 'NZD', label: 'NZD — New Zealand Dollar (NZ$)' }]) },
