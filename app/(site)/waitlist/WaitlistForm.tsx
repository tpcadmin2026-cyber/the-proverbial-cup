...
export function WaitlistForm({ siteName, heading, subheading, successMessage, showPosition }: Props) {
    ...
    >
      <div className="max-w-lg mx-auto w-full">
        ...
        <div className="bg-white border border-[#c8c4a8] rounded-lg shadow-md overflow-hidden">
          ...
          {status === 'success' ? (
            <div className="px-8 py-12 text-center">
              ...
              {alreadyOnList ? (
                <>
                  <p className="font-playfair text-xl text-[#35291C] mb-2">Already Registered</p>
                  <p className="font-baskerville italic text-[#4B4C44] text-lg">Your card is already on our list. We shall write when the time comes.</p>
                </>
              ) : (
                <>
                  <p className="font-playfair text-xl text-[#35291C] mb-2">Card Received</p>
                  <p className="font-baskerville italic text-[#4B4C44] text-lg">{successMessage}</p>
                  {showPosition && position && (
                    <div className="mt-6 inline-block border border-[#C4AB77] rounded px-6 py-3 text-center">
                      <div className="text-xs text-[#C4AB77] uppercase tracking-widest mb-1">Your position</div>
                      <div className="font-playfair text-3xl text-[#35291C]">#{position}</div>
                    </div>
                  )}
                </>
              )}
            ...
            <form onSubmit={handleSubmit} className="px-8 py-8 space-y-5">
              ...
              <div>
                <label className="block text-xs font-semibold text-[#4B4C44] uppercase tracking-wider mb-1.5">
                  Anything to add? <span className="text-xs font-normal text-gray-400">(optional)</span>
                </label>
                <textarea
                  rows={3}
                  value={form.message}
              ...
              >
                {status === 'submitting' ? 'Registering…' : 'Reserve My Place'}
              </button>
            </form>
          )}
        </div>
        ...
        <div className="text-center mt-8 text-xs text-[#4B4C44]">
          <a href="/" className="text-[#C4AB77] hover:underline">← Return to The Gazette</a>
        </div>
      </div>
    </div>
  )
